...
function MyReports() {
  ...
  const loadMyReports = useCallback(async (userId) => {
    if (!userId) {
      setReports([]);
      setLoading(false);
      return;
    ...
    try {
      ...
      const reportsQuery = query(
        collection(db, "reports"), // Change to "incidents" if your collection name is incidents
        where("userId", "==", userId)
      );
      ...
      <main className="my-reports-page">
        <header className="my-reports-header">
          <div>
            <h1>📄 My Reports</h1>
            <p>
              View, edit or delete reports submitted by your account.
            </p>
          </div>
          ...
          <button
            ...
            onClick={() => currentUser && loadMyReports(currentUser.uid)}
          >
            🔄 Refresh
          </button>
        </header>
              ...
              return (
                <article className="my-report-card" key={report.id}>
                  <div className="my-report-card-top">
                    <div>
                      <h2>📍 {report.area || "Unknown area"}</h2>
                      <p>
                  ...
                  {editing ? (
                    ...
                    <>
                      ...
                      <div className="my-report-meta">
                        ...
                        {report.updatedAt && (
                          <span>
                            ✏️ Edited {formatDate(report.updatedAt)}
                          </span>
                        )}
                      </div>
